typedef uint32_t typeoff;
typedef uint32_t nameoff;
typedef uint32_t textoff;
...
struct Go_pclntab {
  // Based on moduledataverify1() at https://golang.org/src/runtime/symtab.go
...
struct Go_moduledata {
  ...
  Go_SliceHeader pclntable;		// []byte pclntable
  Go_SliceHeader ftab;			  // []functab
  Go_SliceHeader filetab;			// []uint32
  ...
  Go_StringHeader pluginpath;
  Go_SliceHeader pkghashes;		// []modulehash

  Go_StringHeader modulename;
  Go_SliceHeader modulehashes;// []modulehash

  uint8_t hasmain; 				    // 1 if module contains the main function, 0 otherwise

  Go_bitvector gcdatamask;
  Go_bitvector gcbssmask;

  uintptr typemap; 				    // typemap map[typeOff]*_type
  uint8_t bad;					      // module failed to load and should be ignored
